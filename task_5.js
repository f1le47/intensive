...
Promise.resolve().then(() => console.log("3")); // Выполнится третьим, так как промис зарезолвится, после чего then вызовется и отдаст колбэк в очередь микротасок,
                                                // когда eventloop увидит, что callstack пуст, то перейдет к очереди микротасок и отправит колбэк в callstack (3)

console.log("4"); // Выполнится вторым, так как колбэки setTimeout и then уйдут в WEB API и микротаски соответственно (2)
